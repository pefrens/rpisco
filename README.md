...
- **Descarga inteligente y caché persistente:** Descarga directa desde
  ...
  automáticos (`httr2::req_retry`) y verificación de integridad MD5.
  Compatible con directivas CRAN (`tools::R_user_dir`).
...
- **Recorte espacial integrado:** Recorte directo en
  `pisco_read(..., aoi = ...)` o con `pisco_clip()` usando polígonos
  `sf`, objetos `bbox` o coordenadas límites
...
- **Métricas estadísticas de validación:** Implementa las métricas
  ...
  refinado de concordancia ($d_r$ de Willmott et al., 2012), Sesgo Medio
  Normalizado (NMB, %) y Error Bruto Medio Normalizado (NMGE).
...
La función `pisco_read()` busca el archivo en la caché local. Si aún no
está descargado, lo descarga automáticamente validando su hash MD5:
